...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	int height = grid.size();
 	...
 	float centerProbability = 1.0 - blurring;
  	float cornerProbability = blurring / 12.0;
  	float adjacentProbability = blurring / 6.0;
  
  	vector < vector <float> > window (3, vector <float> (3, 0));
  
  	window[0][0] = cornerProbability;
	window[0][1] = adjacentProbability;
	window[0][2] = cornerProbability;
  
  	window[1][0] = adjacentProbability;
	window[1][1] = centerProbability;
	window[1][2] = adjacentProbability;
  
  	window[2][0] = cornerProbability;
	window[2][1] = adjacentProbability;
	window[2][2] = cornerProbability;
  
  	newGrid = zeros(height, width);
    
  	for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        float entry = grid[i][j];
        for (int dx = -1; dx < 2; dx++) {
          for (int dy = -1; dy < 2; dy++) {
            float multiply = window[dx + 1][dy + 1];
            newGrid[(i + dy + height) % height][(j + dx + width) % width] += multiply * entry;
          }
        }
      }
    }
